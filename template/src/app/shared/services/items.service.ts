...
function sort(tableItem: Items[], column: SortColumn, direction: string): Items[] {
    if (direction === '' || column === '') {
        return tableItem;
    } else {
...
function matches(table: Items, term: string, pipe: PipeTransform) {
    return table.item_Descripcion.toLowerCase().includes(term.toLowerCase());
  }
