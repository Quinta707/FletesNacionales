...
import { Departamentos } from '../model/departamento.model';
...
function sort(tableItem: Departamentos[], column: SortColumn, direction: string): Departamentos[] {
    if (direction === '' || column === '') {
        return tableItem;
    } else {
...
function matches(table: Departamentos, term: string, pipe: PipeTransform) {
  return table?.depa_Nombre.toLowerCase().includes(term.toLowerCase());
}
...
export class TableService {
  ...
  DeleteDepartamentos(Departamentos: Departamentos){
    return this.http.put<Departamentos[]>(this.DepartamentosDelete,Departamentos)
  }


  DepartamentosEditar = Global + "Departamentos/Editar";
  EditarDepartamentos(Departamentos:Departamentos){
    return this.http.put<Departamentos[]>(this.DepartamentosEditar,Departamentos)
  }
