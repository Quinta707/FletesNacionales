...
function sort(tableItem: Sucursales[], column: SortColumn, direction: string): Sucursales[] {
    if (direction === '' || column === '') {
        return tableItem;
    } else {
...
function matches(table: Sucursales, term: string, pipe: PipeTransform) {
    return table.sucu_Nombre.toLowerCase().includes(term.toLowerCase());
  }
