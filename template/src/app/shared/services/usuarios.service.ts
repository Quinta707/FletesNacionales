...
function sort(tableItem: Usuarios[], column: SortColumn, direction: string): Usuarios[] {
    if (direction === '' || column === '') {
        return tableItem;
    } else {
...
function matches(table: Usuarios, term: string, pipe: PipeTransform) {
    return table.user_NombreUsuario.toLowerCase().includes(term.toLowerCase());
}
