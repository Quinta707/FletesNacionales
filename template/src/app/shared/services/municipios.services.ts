...
function sort(tableItem: Municipios[], column: SortColumn, direction: string): Municipios[] {
    if (direction === '' || column === '') {
        return tableItem;
    } else {
...
function matches(table: Municipios, term: string, pipe: PipeTransform) {
    return table.muni_Nombre.toLowerCase().includes(term.toLowerCase());
  }
