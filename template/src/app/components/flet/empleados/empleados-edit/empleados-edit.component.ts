...
import { Empleados } from 'src/app/shared/model/empleados.model';


@Component({
  selector: 'app-empleados-edit',
  templateUrl: './empleados-edit.component.html',
  styleUrls: ['./empleados-edit.component.scss']
})
export class EmpleadosEditComponent {
  ...
  Editar(){
    ...
    .subscribe((data:any)=> {
      ...
      this.FormGroup1.get('empe_Identidad').setValue(data.empe_Identidad);
      this.FormGroup1.get('empe_FechaNacimiento').setValue(data.empe_FechaNacimiento.toString().replace('T00:00:00', ''));
      this.FormGroup1.get('empe_Sexo').setValue(data.empe_Sexo);
      this.FormGroup1.get('eciv_Id').setValue(data.eciv_Id);
      this.FormGroup1.get('muni_Id').setValue(data.muni_Id);
      this.FormGroup1.get('empe_Telefono').setValue(data.empe_Telefono);
      this.FormGroup1.get('carg_Id').setValue(data.carg_Id);
      this.FormGroup1.get('sucu_Id').setValue(data.sucu_Id);
      this.FormGroup1.get('empe_DireccionExacta').setValue(data.empe_DireccionExacta);
    })
  }
  public Guardar() {
